...
public class TicketReleasedEventHandlerTests
{
    ...
    private static TicketReleasedEventHandler CreateHandler(User? user, Ticket? ticket, SocialEvent? socialEvent, IEmailSender emailSender)
    {
    ...
    public async Task TicketReleased_ForResolvedData_SendsEmail()
    {
        ...
        var domainEvent = new TicketReleasedEvent(ticket.Id, user.Id, eventId);
    ...
    [Fact]
    public async Task TicketReleased_ForNotFoundUser_DoesNotSendEmail()
    {
        // Arrange
        var eventId = SocialEventId.CreateUnique();
        ...
        var domainEvent = new TicketReleasedEvent(ticket.Id, userId, eventId);
    ...
    [Fact]
    public async Task TicketReleased_WhenSenderThrows_HandlesException()
    {
        // Arrange
        var eventId = SocialEventId.CreateUnique();
        var user = User.Register("user@test.com", "Test", "User", "123456789");

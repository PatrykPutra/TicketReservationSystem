...
public class TicketConfirmedEventHandlerTests
{
    ...
    private static (TicketConfirmedEventHandler Handler, Mock<IEmailSender> EmailSender) CreateHandler(
        User? user,
        Ticket? ticket,
    ...
    {
        var (eventId, ticketId, userId) = CreateSeededData(out var user, out _);
        var (handler, emailSender) = CreateHandler(user, ticket: null, socialEvent: null);
